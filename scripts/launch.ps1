...
# with the first argument being the name of the .tex file
# (except for rapport_para.tex, which is the main report file)
...
foreach ($texFile in $texFiles) {
	if ($texFile.Name -ne "rapport_para.tex") {
		...
		python .\scripts\citationcheck.py $fileName 1
